...
/** Keys of the uploads collected on the docs step. */
type DocKey =
  | 'photo' | 'signature' | 'aadhaarCard' | 'education'
  | 'category_cert' | 'domicile' | 'resume' | 'idProof';

/** The persisted application draft. Values are the raw strings the inputs hold. */
interface FormState {
  step: number;
  positionId: string;
  /** Derived from the selected position; null on the direct track. */
  exam: ExamId | null;
  mobile: string; otpVerified: boolean;
  aadhaar: string; kycConsent: boolean; fullName: string; dob: string; gender: string; kycVerified: boolean;
  parentName: string; category: string; pwd: string; email: string; division: string; district: string;
  tehsil: string; village: string; pincode: string; address: string;
  qualification: string; stream: string; passYear: string; percent: string; agriBg: string; experience: string;
  prefDivision: string; willingLocal: string;
  docs: Record<DocKey, string>;
  declTruth: boolean; declAntiCorr: boolean; declTnc: boolean;
  payMethod: string; appId: string;
  /** Set once the docs step has been submitted, so empty slots show their error. */
  _docErr?: boolean;
  /** bindStep() writes fields back by their data-field name, hence the index signature. */
  [key: string]: unknown;
}
...
export const formState: FormState = {
  step: 0,
  positionId: '',
  exam: null,        // derived from the selected position
...
function saveState() {
  try { localStorage.setItem(STORE_KEY, JSON.stringify(formState)); } catch {}
}
...
export function initApply(currentLang: Lang) {
  lang = currentLang;
  loadState();
  renderForm();
}
export function setApplyLang(l: Lang) { lang = l; renderForm(); }

/** Called by the Positions Explorer's "Apply Now" — pre-selects the position and jumps past step 0. */
export function applyToPosition(positionId: string) {
  const position = getPosition(positionId);
  if (!position) return;
  formState.positionId = positionId;
  formState.exam = position.applicationTrack === 'exam' ? position.examId ?? null : null;
  formState.step = 1;
  saveState();
  renderForm();
}
...
type Strings = (typeof STR)['hi'];
function s(): Strings { return STR[lang]; }
...
/** Looks up a label by key, falling back to the key itself for literal labels. */
const label = (key: string) => (STR[lang] as unknown as Record<string, string>)[key] ?? key;

function field(id: string, labelKey: string, value: string, opts: FieldOpts = {}) {
  const L = s();
  ...
  return `<div class="field" id="field_${id}">
    <label for="f_${id}">${esc(label(labelKey))}${req}</label>
    ${input}
    <div class="err">${esc(opts.errMsg || L.err_required)}</div>
  </div>`;
}

function radioField(id: string, labelKey: string, value: string, options: string[]) {
  const L = s();
  ...
  return `<div class="field" id="field_${id}">
    <label>${esc(label(labelKey))} <span class="req">*</span></label>
...
function stepHTML() {
  ...
  if (name === 'docs') {
    ...
    const reqDocs: DocKey[] = examTrack ? ['photo', 'signature', 'aadhaarCard', 'education'] : ['photo', 'resume', 'idProof'];
    const optDocs: DocKey[] = examTrack ? ['category_cert', 'domicile'] : [];
    const cell = (key: DocKey, req: boolean) => {
  ...
  if (name === 'declare') {
    ...
    const g = (rows: [string, string][]) => rows.map(r => `<div class="rg-row"><span class="k">${esc(r[0])}</span><span class="v">${esc(r[1] || '—')}</span></div>`).join('');
    const group = (titleKey: keyof Strings['review_groups'], editStepName: string, rows: [string, string][]) => `
    ...
    const contactRows: [string, string][] = [[L.s1_mobile, formState.mobile]];
...
// ----- validation per step -----
function setErr(id: string, on: boolean) { const el = document.getElementById('field_' + id); if (el) el.classList.toggle('invalid', !!on); }

function validateStep() {
  const name = stepName(), L = s();
  let ok = true;
  const need = (id: string, cond: unknown) => { const bad = !cond; setErr(id, bad); if (bad) ok = false; };
  if (name === 'position') { if (!formState.positionId) { setErr('positionId', true); ok = false; } }
    ...
    const reqDocs: DocKey[] = isExamTrack() ? ['photo', 'signature', 'aadhaarCard', 'education'] : ['photo', 'resume', 'idProof'];
    ok = reqDocs.every(k => Boolean(formState.docs[k]));
    if (!ok) renderForm();
  }
...
async function mockApi(path: string, body?: unknown): Promise<{ success: boolean; [k: string]: unknown }> {
  try {
...
/** The form is re-rendered as an HTML string, so every lookup can miss. */
...
/** Scrolls the apply section back into view after a step change. */
const scrollToApply = () => byId('apply')?.scrollIntoView({ behavior: 'smooth' });
...
function bindStep() {
  ...
  document.querySelectorAll<HTMLInputElement>('[data-field]').forEach(el => {
    ...
    el.addEventListener('change', () => { formState[el.dataset.field as string] = el.value; saveState(); });
  });

  // position dropdown
  const positionSelect = document.getElementById('f_positionId') as HTMLSelectElement | null;
  if (positionSelect) {
    positionSelect.addEventListener('change', () => {
      const position = getPosition(positionSelect.value);
      formState.positionId = positionSelect.value;
      formState.exam = position && position.applicationTrack === 'exam' ? position.examId ?? null : null;
  ...
  // checkboxes
  all<HTMLInputElement>('[data-check]').forEach(el => {
    const key = el.dataset.check as string;
    el.addEventListener('change', () => { formState[key] = el.checked; setErr(key, false); setErr('decl', false); saveState(); });
  });

  // radio chips
  all('[data-radio]').forEach(el => {
    el.addEventListener('click', () => {
      const id = el.dataset.radio as string;
      formState[id] = el.dataset.value as string;
      all(`[data-radio="${id}"]`).forEach(c => c.classList.remove('checked'));
  ...
  if (prev) prev.addEventListener('click', () => { formState.step = Math.max(0, st - 1); saveState(); renderForm(); scrollToApply(); });
  ...
  if (next) next.addEventListener('click', () => {
    ...
    formState.step = st + 1; saveState(); renderForm(); scrollToApply();
  });

  // Step: OTP
  const sendOtp = byId<HTMLButtonElement>('sendOtp');
  ...
  const verifyOtp = byId<HTMLButtonElement>('verifyOtp');
  if (verifyOtp) verifyOtp.addEventListener('click', async () => {
    const otp = byId<HTMLInputElement>('f_otp')?.value ?? '';
  ...
  // Step: KYC
  const verifyKyc = byId<HTMLButtonElement>('verifyKyc');
  ...
  // Step: file uploads
  all('[data-doc]').forEach(box => {
    const key = box.dataset.doc as DocKey;
    const inp = box.querySelector<HTMLInputElement>('[data-doc-input]');
    if (!inp) return;
    box.addEventListener('click', (e) => {
      if ((e.target as HTMLElement | null)?.tagName !== 'INPUT') inp.click();
    });
    inp.addEventListener('change', () => {
      const file = inp.files?.[0]; if (!file) return;
      if (file.size > 5 * 1024 * 1024) { alert(lang === 'hi' ? 'File 5MB se chhoti honi chahiye' : 'File must be under 5MB'); return; }
      const reader = new FileReader();
      reader.onload = () => { formState.docs[key] = String(reader.result ?? ''); saveState(); renderForm(); };
  ...
  all('[data-goto]').forEach(el => {
    el.addEventListener('click', () => { formState.step = parseInt(el.dataset.goto ?? '0', 10); saveState(); renderForm(); scrollToApply(); });
  });

  // Step: payment (exam track only)
  all('[data-pay]').forEach(el => {
    el.addEventListener('click', () => { formState.payMethod = el.dataset.pay ?? ''; all('[data-pay]').forEach(p => p.classList.remove('selected')); el.classList.add('selected'); setErr('pay', false); saveState(); });
  });
  const payNow = byId<HTMLButtonElement>('payNow');
  if (payNow) payNow.addEventListener('click', async () => {
    ...
    formState.appId = typeof r?.appId === 'string' ? r.appId : generateAppId();
    formState.step = currentSteps().length - 1; saveState(); renderForm(); scrollToApply();
  });

  // Confirmation step
  const dlPdf = byId('dlPdf');
  if (dlPdf) dlPdf.addEventListener('click', () => downloadPdf());
  const newApp = byId('newApp');
...
let resendInt: ReturnType<typeof setInterval> | undefined;
...
function downloadPdf() {
  ...
  // Pop-up blockers can refuse the window; nothing to print if so.
  if (!w) return;
  const rows = ([
    ...
    ...(examTrack ? [['Aadhaar', formState.aadhaar ? '••••••••' + formState.aadhaar.slice(-4) : ''] as [string, string]] : []),
  ...
  ] as [string, string][]).map(r => `<tr><td style="padding:8px;border:1px solid #ddd;color:#555">${r[0]}</td><td style="padding:8px;border:1px solid #ddd;font-weight:700">${esc(r[1] || '')}</td></tr>`).join('');
